...
print "Testing YAPLSPD document formatting (with changes)...\n";
...
# Test with messy Perl code that needs formatting
my $messy_code = <<'PERL';
sub messy_function  {
...
# Force basic formatting by adding trailing spaces
my $messy_with_spaces = $messy_code . "    \n  \n";
$doc = TestDocument->new($messy_with_spaces);
...
if ($edits && ref($edits) eq 'ARRAY') {
    ...
    if (@$edits) {
        my $edit = $edits->[0];
        print "Original code (with trailing spaces):\n";
        print "=" x 50 . "\n";
        print $messy_with_spaces;
        print "=" x 50 . "\n";
